...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
	...
	Camera->RelativeLocation = { -0.4f, 2.2f, 2.8f };
	Camera->RelativeRotation = { 0.f, -0.4f, 1.2f };
	...
	XGizmo->SetRasterizerState(RasterizerState::Solid_Culling_None);
	YGizmo->SetRasterizerState(RasterizerState::Solid_Culling_None);
	ZGizmo->SetRasterizerState(RasterizerState::Solid_Culling_None);
	...
	Test4->RelativeLocation = Test2->RelativeLocation + Test2->GetForwardVector_UE() * 0.15f;
	Test4->RelativeRotation = Test2->RelativeRotation;
	Test4->RelativeScale3D = Test4->RelativeScale3D * 0.5f;
	Test4->RelativeScale3D.x = Test4->RelativeScale3D.x * 1.3f;
	Test4->RelativeScale3D.y = Test4->RelativeScale3D.y * 0.5f;
	Test4->RelativeScale3D.z = Test4->RelativeScale3D.z * 0.5f;

	//Console.UE_LOG("Test2 : %s, $s, $s", Test)
				...
				else
				{
					if (pickedGizmoPtr)
					{
						...
						if (AxisScreenLength > 0.00001f)
						{
							...
							Console.UE_LOG("WorldMove : %f, %f, %f", WorldMove.x, WorldMove.y, WorldMove.z);
