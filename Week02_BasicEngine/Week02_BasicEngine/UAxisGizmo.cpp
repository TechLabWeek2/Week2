...
void UAxisGizmo::Render(URenderer* Renderer)
{
	...
	Renderer->UpdateConstant(this->GetModelMatrix() * Renderer->MainCamera->GetViewMatrix() * Renderer->MainCamera->GetProjectionMatrix(), bIsSelected = false);
	...
	Renderer->UpdateConstant(this->GetModelMatrix(), bIsSelected = false);
	Renderer->RenderPrimitive(Vertices, NumVertices);
}
